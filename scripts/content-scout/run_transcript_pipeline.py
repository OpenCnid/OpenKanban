...
def build_steps() -> list[StepSpec]:
    return [
        ...
        StepSpec(name="classify", script="classify_annotate.py"),
        StepSpec(name="merge", script="merge_visuals.py", per_video=True),
        StepSpec(name="summarize", script="summarize_video.py", per_video=True),
...
def build_step_args(step_name: str, pipeline_date: str, preset: dict[str, Any]) -> list[str]:
    if step_name == "select":
        return [
            "--channels",
            "config/transcript-studio/channels.json",
            "--playlists",
            "config/transcript-studio/playlists.json",
            "--log",
            TRANSCRIPT_LOG_PATH,
            "--limit",
            "10",
...
def run_script(script: str, step_name: str, args: list[str], verbose: bool, dry_run: bool) -> tuple[int, str]:
...
def main() -> int:
    ...
    for step in steps:
        ...
        if step.per_video:
            ...
            code, message = run_script(step.script, step.name, step_args, args.verbose, args.dry_run)
